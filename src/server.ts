  ...
  try {
    switch (name) {
      ...
      case "clawtex_update_state": {
        ...
        const result = await client.updateState(
          typedArgs.id,
          typedArgs.type,
          typedArgs.data
        );
      ...
      case "clawtex_approve_lesson": {
        const typedArgs = args as { lesson_id: string };
      ...
      case "clawtex_signal_lesson": {
        ...
        const result = await client.signalLesson(
          typedArgs.lesson_id,
          typedArgs.relevant,
          typedArgs.followed
        );
      ...
      case "clawtex_start_session": {
        const typedArgs = args as { metadata?: Record<string, unknown> };
      ...
      case "clawtex_end_session": {
        const typedArgs = args as { session_id: string; summary?: string };
        const result = await client.endSession(
          typedArgs.session_id,
          typedArgs.summary
        );
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
